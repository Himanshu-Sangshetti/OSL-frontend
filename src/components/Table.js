...
const Table = (probs) => {
  ...
  const showPdf = (file) => {
    window.open(`https://OSL-backend.chickenkiller.com/files/${file}`, "_blank", "noreferrer");
  };

  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://OSL-backend.chickenkiller.com/api/assignment/deleteAssignment/${id}`);
    } catch (error) {
